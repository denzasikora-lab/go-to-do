...
func (s *Service) textCommitTitleEdit(ctx context.Context, u *duser.User, snap *fsm.Snapshot, chatID int64, text string) {
	id, err := fsm.PayloadInt64Required(snap.Payload, fsm.KeyEditTodoID)
	if err != nil {
		s.abortEdit(ctx, u.ID, chatID, snap)
		return
	}
	err = todorepo.UpdateTitle(ctx, s.Pool, u.ID, id, text)
	...
	_ = s.ReplyHTML(chatID, "<b>Title updated.</b>", MainMenuMarkup())
}
